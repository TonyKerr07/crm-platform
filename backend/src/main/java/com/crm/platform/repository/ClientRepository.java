...
public interface ClientRepository extends JpaRepository<Client, UUID> {
    ...
    @Query("""
            ...
            """)
    Page<Client> searchByTerm(@Param("search") String search, Pageable pageable);

    long countByActiveTrue();

    @Query("SELECT COUNT(c) FROM Client c WHERE c.createdAt >= :startDate AND c.createdAt < :endDate")
